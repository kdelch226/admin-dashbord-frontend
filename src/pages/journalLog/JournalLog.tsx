import { Box, List, ListItem, MenuItem, Select, Stack, TextField, Typography } from '@mui/material';
...
import JournalLogCard from '../../components/journalLogs/journalLogCard';
...
const JournalLog: React.FC<ActivitiesProps> = ({ bgcolor, restricted }) => {
  useEffect(() => {
  ...
  const {
    ...
    resource: 'journalLogs',
  });

  const journalLogs = data?.data ?? [];
  const currentFilterValues = useMemo(() => {
    ...
    return {
      userEmail: logicalFilters.find((item) => item.field === "userMail")?.value || "",
  ...
  console.log('journalLogs', journalLogs);
  return (
    <Box
      sx={{
        px: 1,
      ...
      <Stack flexDirection={'row'} p={1} alignItems={'center'}>
        ...
        <Typography p={1} fontWeight={550} fontSize={restricted ? 18:20}>{!journalLogs.length ? 'No Latest Logs' : `Recent Logs (${journalLogs.length})`}</Typography>
      ...
      <Box >
        <List sx={{ width: '100%' }}>
          {journalLogs.map(journal => (
            ...
            >
              <JournalLogCard
                id={journal._id}
                userAvatar={journal.userAvatar}
                userMail={journal.userMail}
        ...
        {journalLogs.length > 0 && !restricted && (
          <Stack
            flexDirection='row'
            alignItems='center'
            mt={3}
